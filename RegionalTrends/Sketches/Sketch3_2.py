...
def build_data():
    ...
    assert mu_phys > 0.0
    assert mu_dyn < 0.0
    np.testing.assert_allclose(mu_phys + mu_dyn, mu_tot, rtol=0.0, atol=1.0e-12)

    return x, T, mu_phys, mu_dyn, mu_tot
...
def make_sketch(output_prefix=None):
    if output_prefix is None:
        output_prefix = OUTPUT_DIR / 'Sketch3_2'
    else:
        output_prefix = Path(output_prefix)
    output_prefix.parent.mkdir(parents=True, exist_ok=True)

    x, T, mu_phys, mu_dyn, mu_tot = build_data()

    y_axis = 0.0
    y_top = np.ceil((max(T[0] + mu_phys, np.max(T)) + 0.45) / 0.5) * 0.5
    ...
    # x-axis ticks for the two periods
    tick_len = 0.045
    ...
    start = (x[0], T[0])
    end_phys = (x[1], T[0] + mu_phys)
    end_dyn = (x[1], T[0] + mu_dyn)
    end_tot = (x[1], T[1])

    # physical = blue warming
    # dynamical = red cooling
