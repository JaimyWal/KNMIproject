...
RED = '#c62828'    # dynamical contribution
BLUE = '#1565c0'   # physical contribution
BLACK = '#111111'
GRID = '0.82'
YEAR_LINE = '0.48'
...
def build_data():
    ...
    # Same total temperature states as Sketch3_2
    T = np.array([1.45, 2.55])

    # Sketch3_2 tendencies
    mu_tot = T[1] - T[0]
    mu_dyn = -1.00
    mu_phys = mu_tot - mu_dyn

    # Sketch3_3 reference tendencies
    mu_ref_tot = 0.0
    mu_ref_dyn = -1.30
    mu_ref_phys = mu_ref_tot - mu_ref_dyn

    # Difference tendencies for Sketch3_4
    muhat_tot = mu_tot - mu_ref_tot
    muhat_dyn = mu_dyn - mu_ref_dyn
    muhat_phys = mu_phys - mu_ref_phys

    assert muhat_dyn > 0.0
    assert muhat_phys > 0.0
    np.testing.assert_allclose(muhat_phys + muhat_dyn, muhat_tot, rtol=0.0, atol=1.0e-12)

    return x, T, muhat_phys, muhat_dyn, muhat_tot
...
def make_sketch(output_prefix=None):
    if output_prefix is None:
        output_prefix = OUTPUT_DIR / 'Sketch3_4'
    else:
        output_prefix = Path(output_prefix)
    output_prefix.parent.mkdir(parents=True, exist_ok=True)

    x, T, muhat_phys, muhat_dyn, muhat_tot = build_data()

    # Keep the same vertical scaling as Sketch3_2.
    # This uses the original 3_2 physical contribution, not the smaller hat contribution.
    y_axis = 0.0
    mu_phys_3_2 = 2.10
    y_top = np.ceil((max(T[0] + mu_phys_3_2, np.max(T)) + 0.45) / 0.5) * 0.5
    ...
    start = (x[0], T[0])
    end_phys = (x[1], T[0] + muhat_phys)
    end_dyn = (x[1], T[0] + muhat_dyn)
    end_tot = (x[1], T[1])

    # All three arrows start at the same point, as in Sketch3_2.
    # The vertical increments close exactly:
    # muhat_phys + muhat_dyn = muhat_tot.
    ...
    place_state_label(ax, x[0], T[0], r'$\langle T\rangle_{P_1}$', dx_points=-20, dy_points=0, fontsize=15)
    place_state_label(ax, x[1], T[1], r'$\langle T\rangle_{P_2}$', dx_points=0, dy_points=10, fontsize=15)
