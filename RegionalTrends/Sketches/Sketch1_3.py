...
def build_data():
    ...
    assert D_dyn > 0.0
    assert D_phys < 0.0
    np.testing.assert_allclose(D_dyn + D_phys, D_tot, rtol=0.0, atol=1.0e-12)

    return x, T, D_dyn, D_phys, D_tot


def label_parts(kind, day):
    main = rf'$D^{{\mathrm{{{kind}}}}}$'
    coord = rf'$({day})$'
    return main, coord
...
def make_sketch(output_prefix=None):
    ...
    x, T, D_dyn, D_phys, D_tot = build_data()

    y_axis = 0.0
    y_top = np.ceil((max(T[0] + D_dyn, np.max(T)) + 0.45) / 0.5) * 0.5
    ...
    for xx, label in zip(x, [r'$d$', r'$d\!+\!1$']):
    ...
    start = (x[0], T[0])
    end_dyn = (x[1], T[0] + D_dyn)
    end_phys = (x[1], T[0] + D_phys)
    end_tot = (x[1], T[1])
    ...
    place_state_label(ax, x[0], T[0], r'$\langle T(d)\rangle$', dx_points=-26, dy_points=0, fontsize=15)
    place_state_label(ax, x[1], T[1], r'$\langle T(d\!+\!1)\rangle$', dx_points=0, dy_points=10, fontsize=15)

    ax.text(1.0, y_top + 0.1, r'Day $d$', ha='center', va='top', fontsize=28)
    ax.text(2.0, y_top + 0.1, r'Day $d\!+\!1$', ha='center', va='top', fontsize=28)
    ax.text(1.5, -0.45, 'Day', ha='center', va='top', fontsize=20)
