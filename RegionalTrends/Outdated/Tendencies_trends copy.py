...
# Keep the full adjacent tendency series for Franks attribution before trimming
# the helper year needed for the final y -> y+1 annual pair.
var_tendencies_franks = {var: data.copy() for var, data in var_tendencies.items()}
var_temp_close_franks = None if var_temp_close is None else var_temp_close.copy()
...
#%% ============================================================================
#   PHASE 4: FRANKS ATTRIBUTION METHOD
#   ============================================================================

FRANKS_SEASON_MONTHS = {
    'DJF': [12, 1, 2],
    'MAM': [3, 4, 5],
    'JJA': [6, 7, 8],
    'SON': [9, 10, 11],
}
FRANKS_PANEL_KEYS = list(FRANKS_SEASON_MONTHS.keys())

def franks_panel_title(panel_key):
    return panel_key

def build_franks_interval_weights(tendency_year, interval):
    time_index = pd.DatetimeIndex(tendency_year['time'].values)
    if interval in {'Raw', 'Daily'}:
        weight_vals = np.ones(time_index.size, dtype='float32')
    elif interval == 'Monthly':
        weight_vals = np.asarray(time_index.days_in_month, dtype='float32')
    elif interval == 'Seasonal':
        ...
        raise ValueError(f'Unsupported proc_freq for Franks attribution: {interval}')

    return xr.DataArray(
        (weight_vals / np.sum(weight_vals, dtype='float64')).astype('float32'),
        dims=['time'],
        coords={'time': tendency_year['time']},
    )

def sum_franks_seasons(seasonal_dict):
    total = 0
    for season in FRANKS_PANEL_KEYS:
        total = total + seasonal_dict[season]
    return total.astype('float32')

def compute_franks_yearly_tendency(tendency_da, proc_freq):
    yearly_times = []
    yearly_total_vals = []
    yearly_season_vals = {season: [] for season in FRANKS_PANEL_KEYS}
    empty = tendency_da.isel(time=slice(0, 0)).assign_coords(time=pd.DatetimeIndex([])).astype('float32')

    for year in years:
        tendency_early = tendency_da.where(tendency_da['time'].dt.year == year, drop=True).sortby('time')
        tendency_late = tendency_da.where(tendency_da['time'].dt.year == year + 1, drop=True).sortby('time')

        if tendency_early.sizes.get('time', 0) == 0 or tendency_late.sizes.get('time', 0) == 0:
            continue
        if not np.all(np.isfinite(tendency_early.values)) or not np.all(np.isfinite(tendency_late.values)):
            continue

        weights_early = build_franks_interval_weights(tendency_early, proc_freq)
        weights_late = build_franks_interval_weights(tendency_late, proc_freq)

        w_early = xr.DataArray(
            np.cumsum(weights_early.values, dtype='float64').astype('float32'),
            dims=['time'],
            coords={'time': tendency_early['time']},
        )
        w_late = xr.DataArray(
            (
                np.cumsum(weights_late.values[::-1], dtype='float64')[::-1]
                - weights_late.values
            ).astype('float32'),
            dims=['time'],
            coords={'time': tendency_late['time']},
        )

        # Earlier-year intervals use the accumulated head weight up to interval i.
        # Later-year intervals use the remaining tail weight after interval i,
        # so the final late-year adjacent interval gets zero weight automatically.
        signed_late = (w_late * tendency_late).astype('float32')
        signed_early = (w_early * tendency_early).astype('float32')
        yearly_total = (signed_late.sum('time', skipna=False) + signed_early.sum('time', skipna=False)).astype('float32')

        if not np.all(np.isfinite(yearly_total.values)):
            continue

        yearly_times.append(pd.Timestamp(year=year, month=1, day=1))
        yearly_total_vals.append(yearly_total)

        for season, season_months in FRANKS_SEASON_MONTHS.items():
            contrib_early_season = signed_early.where(
                tendency_early['time'].dt.month.isin(season_months),
                other=0.0,
            ).sum('time', skipna=False)
            contrib_late_season = signed_late.where(
                tendency_late['time'].dt.month.isin(season_months),
                other=0.0,
            ).sum('time', skipna=False)
            yearly_season_vals[season].append((contrib_early_season + contrib_late_season).astype('float32'))

    if len(yearly_total_vals) == 0:
        return empty, {season: empty.copy() for season in FRANKS_PANEL_KEYS}

    yearly_coord = xr.IndexVariable('time', pd.DatetimeIndex(yearly_times))
    yearly_total = xr.concat(yearly_total_vals, dim=yearly_coord).astype('float32')
    yearly_by_season = {season: xr.concat(yearly_season_vals[season], dim=yearly_coord).astype('float32')
                        for season in FRANKS_PANEL_KEYS}

    return yearly_total, yearly_by_season

def compute_franks_metrics(data):
    if data is None:
        return None

    vals = np.asarray(data.values, dtype='float64')
    ...
    return {
        'n': int(vals_finite.size),
        'rmse': float(np.sqrt(np.mean(vals_finite**2))),
        'max_abs': float(np.max(np.abs(vals_finite))),
        'mean_abs': float(np.mean(np.abs(vals_finite))),
    }

print('='*60)
print('PHASE 4: Franks yearly attribution')
print('='*60)

frank_yearly_total = {}
frank_yearly_seasonal = {}
for var in vars_tendencies:
    print(f'  Franks attribution: {var}')
    yearly_total, yearly_by_season = compute_franks_yearly_tendency(var_tendencies_franks[var], proc_freq)
    frank_yearly_total[var] = yearly_total
    frank_yearly_seasonal[var] = yearly_by_season

frank_temp_yearly_total = None
frank_temp_yearly_seasonal = None
if var_temp_close_franks is not None:
    print('  Franks attribution: temperature change')
    frank_temp_yearly_total, frank_temp_yearly_seasonal = compute_franks_yearly_tendency(var_temp_close_franks, proc_freq)

franks_group_panel_data = {
    group_name: {
        season: build_group_panel_data(
            group_name,
            {var: frank_yearly_seasonal[var][season] for var in vars_tendencies}
        )
        for season in FRANKS_PANEL_KEYS
    }
    for group_name in var_tend_main
}
franks_group_yearly_data = {
    group_name: build_group_panel_data(group_name, frank_yearly_total)
    for group_name in var_tend_main
}
franks_closure_panel_data = None
if frank_temp_yearly_seasonal is not None:
    franks_closure_panel_data = {
        season: build_closure_panel_data(
            {var: frank_yearly_seasonal[var][season] for var in vars_tendencies},
            frank_temp_yearly_seasonal[season],
        )
        for season in FRANKS_PANEL_KEYS
    }

franks_closure_data = None
franks_closure_metrics = {}
franks_closure_reasonable = None
franks_yearly_points = 0

if var_tend_close_true in frank_yearly_total:
    franks_yearly_points = int(frank_yearly_total[var_tend_close_true]['time'].size)

if var_temp_close_yearly is not None and var_tend_close_true in frank_yearly_total:
    franks_true_tendency = frank_yearly_total[var_tend_close_true]
    franks_true_season_sum = sum_franks_seasons(frank_yearly_seasonal[var_tend_close_true])
    franks_summed_processes = sum_tendency_terms(frank_yearly_total, var_tend_close_pr)

    temp_close_aligned, franks_true_aligned, franks_true_season_aligned, franks_summed_processes_aligned = xr.align(
        var_temp_close_yearly.astype('float32'),
        franks_true_tendency.astype('float32'),
        franks_true_season_sum.astype('float32'),
        franks_summed_processes.astype('float32'),
        join='inner',
    )

    franks_closure_data = {
        'temp_close': temp_close_aligned.astype('float32'),
        'true_tendency': franks_true_aligned.astype('float32'),
        'summed_processes': franks_summed_processes_aligned.astype('float32'),
        'seasonal_sum': franks_true_season_aligned.astype('float32'),
        'diff_true': (temp_close_aligned - franks_true_aligned).astype('float32'),
        'diff_sum': (temp_close_aligned - franks_summed_processes_aligned).astype('float32'),
        'diff_seasonal_sum': (temp_close_aligned - franks_true_season_aligned).astype('float32'),
        'seasonal_residual': (franks_true_season_aligned - franks_true_aligned).astype('float32'),
    }

    franks_closure_metrics = {
        'diff_true': compute_franks_metrics(franks_closure_data['diff_true']),
        'diff_sum': compute_franks_metrics(franks_closure_data['diff_sum']),
        'diff_seasonal_sum': compute_franks_metrics(franks_closure_data['diff_seasonal_sum']),
        'seasonal_residual': compute_franks_metrics(franks_closure_data['seasonal_residual']),
    }

    actual_vals = np.asarray(franks_closure_data['temp_close'].values, dtype='float64')
    actual_finite = actual_vals[np.isfinite(actual_vals)]
    if actual_finite.size > 0 and franks_closure_metrics['diff_true'] is not None and franks_closure_metrics['seasonal_residual'] is not None:
        actual_scale = float(np.max(np.abs(actual_finite)))
        diff_tol = max(2e-2, 0.10 * actual_scale)
        franks_closure_reasonable = (
            franks_closure_metrics['diff_true']['rmse'] <= diff_tol
            and franks_closure_metrics['seasonal_residual']['max_abs'] <= 1e-4
        )
    else:
        franks_closure_reasonable = None

print(f'  Produced {franks_yearly_points} Franks yearly points for {proc_freq} intervals')
...
def plot_panel_grid(panel_series_dict, layout, panel_width, panel_height, y_range, shared_labels,
                    ...
                    panel_keys=None, panel_title_func=None):
    ...
    for idx, month_key in enumerate(panel_keys):
        ...
        panel_title = format_panel_title(month_key) if panel_title_func is None else panel_title_func(month_key)
        format_time_axis(ax, panel_title, title_fs, tick_fs, zero_line=zero_line)
...
#%% ============================================================================
#   PHASE 5: PLOTTING TENDENCY GROUPS
#   ============================================================================
...
for group_name in var_tend_main:
    main_var = var_tend_main[group_name]
    group_unit = var_units_cfg.get(main_var, '')
    seasonal_group_label = (
        f'Seasonal contribution to yearly tendency ({group_unit})'
        if group_unit else
        'Seasonal contribution to yearly tendency'
    )
    franks_panel_series = {
        season: group_plot_series(group_name, franks_group_panel_data[group_name][season])
        for season in FRANKS_PANEL_KEYS
    }
    plot_panel_grid(
        franks_panel_series,
        ...
        mirror_y_axes=group_mirror_y_axes,
        use_year_axis=True,
        y_label=seasonal_group_label,
        save_var=main_var,
        suffix='franks_group_seasonal',
        zero_line=True,
        panel_keys=FRANKS_PANEL_KEYS,
        panel_title_func=franks_panel_title,
    )

    yearly_group_label = f'Franks yearly attribution ({group_unit})' if group_unit else 'Franks yearly attribution'
    franks_yearly_series = group_plot_series(group_name, franks_group_yearly_data[group_name])
    plot_single_panel(
        franks_yearly_series,
        panel_width=group_panel_width,
        panel_height=group_panel_height,
        y_range=group_y_range,
        shared_labels=group_shared_labels,
        y_label=yearly_group_label,
        save_var=main_var,
        suffix='franks_group_yearly',
        title='Franks yearly attribution',
        zero_line=True,
    )

#%% ============================================================================
#   PHASE 6: PLOTTING / CHECKING CLOSURE
#   ============================================================================
...
franks_close_label = (
    f'Seasonal contribution to yearly tendency ({close_unit})'
    if close_unit else
    'Seasonal contribution to yearly tendency'
)
...
franks_closure_actual_series = None
if franks_closure_panel_data is not None:
    franks_closure_actual_series = {
        season: closure_actual_plot_series(franks_closure_panel_data[season])
        for season in FRANKS_PANEL_KEYS
    }

if franks_closure_actual_series is not None:
    plot_panel_grid(
        franks_closure_actual_series,
        layout=(2, 2),
        ...
        use_year_axis=True,
        y_label=franks_close_label,
        save_var=var_tend_close_true,
        suffix='franks_closure_seasonal',
        panel_keys=FRANKS_PANEL_KEYS,
        panel_title_func=franks_panel_title,
    )
if franks_closure_data is not None:
    franks_closure_actual_yearly_series = closure_actual_plot_series(franks_closure_data)
    plot_single_panel(
        franks_closure_actual_yearly_series,
        panel_width=close_panel_width,
        panel_height=close_panel_height,
        y_range=close_y_range,
        shared_labels=close_shared_labels,
        y_label=franks_close_label,
        save_var=var_tend_close_true,
        suffix='franks_closure_actual_yearly',
        title='Yearly',
    )
...
print('='*60)
print('FRANKS SUMMARY')
print('='*60)
print(f'proc_freq: {proc_freq}')
print(f'yearly points: {franks_yearly_points}')
if franks_closure_reasonable is None:
    print('closure against annual-mean temperature change looks reasonable: unknown')
else:
    closure_label = 'yes' if franks_closure_reasonable else 'no'
    print(f'closure against annual-mean temperature change looks reasonable: {closure_label}')

if franks_closure_metrics.get('diff_true') is not None:
    print(
        '  tendtot vs annual mean change: '
        f"rmse={franks_closure_metrics['diff_true']['rmse']:.4f}, "
        f"max_abs={franks_closure_metrics['diff_true']['max_abs']:.4f}"
    )
if franks_closure_metrics.get('seasonal_residual') is not None:
    print(
        '  seasonal sum minus tendtot: '
        f"max_abs={franks_closure_metrics['seasonal_residual']['max_abs']:.6f}"
    )
