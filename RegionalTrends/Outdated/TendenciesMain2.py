...
file_freq = 'Seasonal'
proc_freq = 'Seasonal'
tend_trans = 'yearlyB'  # options: 'A', 'cont', 'yearlyB'
save_name_base = None
...
tend_is_transition = tend_trans == 'A'
tend_is_yearlyB = tend_trans == 'yearlyB'
if tend_trans not in ['A', 'cont', 'yearlyB']:
    raise ValueError("tend_trans must be one of 'A', 'cont', or 'yearlyB'")
...
if file_freq in ['Monthly', 'Seasonal']:
    ...
    def build_file_cfg_select(freq_str):
        ...
        if freq_str in ['Monthly', 'Seasonal']:
            suffix = {
                'Monthly': {'A': 'monthlyA', 'cont': 'monthlycont', 'yearlyB': 'monthlyyearlyB'}[tend_trans],
                'Seasonal': {'A': 'seasonalA', 'cont': 'seasonalcont', 'yearlyB': 'seasonalyearlyB'}[tend_trans],
            }[freq_str]
...
# Option for anomalising A (nog niet, willen we dit wel?)
# Function to compare actual temperature differences to tendency sums and tendtot after processing
# Option to do these things yearly as well (new yearly function though)
# Raw seasonal contributions C.

# TREND ATTRIBUTION:
# Function to construct C (using either anomalised A or not)
# Option for weights and q
# Function to compute THETA in 3 different formulas
# Function to compute theta
# Option for reference value or not and which reference
