...
file_freq = 'Daily'
tendency_input = 'cont'  # options: 'A', 'cont'
periods = {'Pref': [1961, 1980], 'PrefL': [1961, 1970], 'PrefR': [1971, 1980], 
           'Ptrend': [1981, 2020], 'PtrendL': [1981, 2000], 'PtrendR': [2001, 2020],}
rolling_window = 91
...
l1_source = 'RACMO2.4A'
tendency_input = tendency_input.lower()
if tendency_input not in ['a', 'cont']:
    raise ValueError("tendency_input must be 'A' or 'cont'")
...
def build_file_cfg_A(freq_str):
    ...
    tendency_suffix = {
        'a': freq_suffix,
        'cont': {'Daily': 'dailycont', 'Monthly': 'monthlycont', 'Seasonal': 'seasonalcont'}[freq_str],
    }[tendency_input]
    ...
    for var in Constants.TENDENCY_VARS:
        cfg['RACMO2.4A'][var] = f'/nobackup_1/users/walj/racmo24/{freq_str}/{var}_{tendency_suffix}_KEXT12_RACMO24p1v7_FINAL_enbud_fix_*.nc'
    return cfg
...
def area_average(da, data_source):
    sample = da.squeeze()
    for dim in ['time', 'year', 'month_day']:
...
def select_period(da, period):
    ...
    if 'season_year' in da.coords:
        return da.where(da['season_year'].isin(years_period), drop=True).astype('float32')
...
def to_cont_year_monthday(da):
    if {'year', 'month_day'}.issubset(da.dims):
        extra_dims = [dim for dim in da.dims if dim not in ['year', 'month_day']]
        return reorder_month_day(da.transpose('year', 'month_day', *extra_dims).astype('float32'))

    year = da['season_year'] if 'season_year' in da.coords else analysis_year(da)
    year = xr.DataArray(year.data, dims='time', coords={'time': da['time']})
    month_day = xr.DataArray(da['time'].dt.strftime('%m-%d').data, dims='time', coords={'time': da['time']})
    out = (
        da.assign_coords(year=year, month_day=month_day)
          .set_index(time=['year', 'month_day'])
...
def determine_mu(A_time, period_left, period_right, split='day'):
    if tendency_input == 'cont':
        C_y_d = to_cont_year_monthday(A_time)
        kappa = weights_kappa(period_left, period_right)
        kappa_left, kappa_right = split_kappa(kappa, period_left, period_right)

        def weighted_curve(kappa_side):
            keep = [year for year in kappa_side['year'].values if year in C_y_d['year'].values]
            return aggregate_calendar(
                (C_y_d.sel(year=keep) * kappa_side.sel(year=keep)).sum('year'),
                split=split,
            )

        curve_left = weighted_curve(kappa_left)
        curve_right = weighted_curve(kappa_right)
        curve_total = (curve_left + curve_right).astype('float32')

        return {
            'mu_total': curve_total.sum().astype('float32'),
...
period_handles = [
    Line2D([0], [0], color='gray', linestyle='solid', label='1981-2000'),
    Line2D([0], [0], color='gray', linestyle='dashed', label='2001-2020'),
]
