...
from RegionalTrends.Helpers.ComputeTendencies_old import construct_tendency, full_interval_time
...
build_file_cfg_orig = ProcessVar.build_file_cfg

def build_file_cfg_A_only(freq_str):
    file_cfg = build_file_cfg_orig(freq_str)

    if freq_str not in {'Monthly', 'Seasonal'}:
        return file_cfg

    racmo_key = f'RACMO2.4A_{freq_str}'
    file_tag = 'monthlyA' if freq_str == 'Monthly' else 'seasonalA'
    base_dir = Path(ProcessVar.Paths.BASE_DIR_CFG[racmo_key])

    for var in tendency_vars:
        file_cfg['RACMO2.4A'][var] = str(base_dir / f'{var}_{file_tag}_*.nc')

    return file_cfg

ProcessVar.build_file_cfg = build_file_cfg_A_only
...
def raw_season_groups(time_coord, months, years):
    ...
    month_start = int(months_arr[0])
    month_end = int(months_arr[-1])
    spans_year_boundary = month_start > month_end

    groups = {}
    for year in np.asarray(years, dtype=int):
        if spans_year_boundary:
            mask = (
                ((year_vals == year - 1) & np.isin(month_vals, months_arr) & (month_vals >= month_start))
                | ((year_vals == year) & np.isin(month_vals, months_arr) & (month_vals <= month_end))
            )
        else:
            mask = (year_vals == year) & np.isin(month_vals, months_arr)

        idx = np.where(mask)[0]
        if idx.size == 0:
            continue
        if idx[-1] - idx[0] + 1 != idx.size:
            raise ValueError(f'Non-contiguous raw season block for {months} in climate year {year}')
...
def raw_seasonal_yearly_from_shifted_tendency(raw_da, months, years):
    groups = raw_season_groups(raw_da['time'], months, np.append(years, years[-1] + 1))
    out_vals = []
    out_years = []

    raw64 = raw_da.astype('float64')

    for year in np.asarray(years, dtype=int):
        if year not in groups or year + 1 not in groups:
            continue

        group0 = groups[year]
        group1 = groups[year + 1]
        i0, N0 = group0['start'], group0['N']
        j0, N1 = group1['start'], group1['N']

        start_term = raw64.isel(time=slice(i0, j0)).sum('time', skipna=False)

        inner0_slice = raw64.isel(time=slice(i0, i0 + N0 - 1))
        inner0_w = xr.DataArray(
            np.arange(N0 - 1, 0, -1, dtype='float64') / N0,
            dims=['time'],
            coords={'time': inner0_slice['time']},
        )
        inner0 = (inner0_slice * inner0_w).sum('time', skipna=False)

        inner1_slice = raw64.isel(time=slice(j0, j0 + N1 - 1))
        inner1_w = xr.DataArray(
            np.arange(N1 - 1, 0, -1, dtype='float64') / N1,
            dims=['time'],
            coords={'time': inner1_slice['time']},
        )
        inner1 = (inner1_slice * inner1_w).sum('time', skipna=False)

        out_vals.append(start_term + inner1 - inner0)
        out_years.append(int(year))

    if not out_vals:
        return None

    return xr.concat(out_vals, dim=xr.IndexVariable('time', pd.to_datetime(dict(year=out_years, month=1, day=1))))


def raw_temperature_seasonal_yearly_change(temp_raw, months, years):
    groups = raw_season_groups(temp_raw['time'], months, np.append(years, years[-1] + 1))
    out_vals = []
    out_years = []

    temp64 = temp_raw.astype('float64')
    ...
    for year in np.asarray(years, dtype=int):
        ...
        group0 = groups[year]
        group1 = groups[year + 1]
        mean0 = temp64.isel(time=slice(group0['start'], group0['stop'])).mean('time', skipna=False)
        mean1 = temp64.isel(time=slice(group1['start'], group1['stop'])).mean('time', skipna=False)

        out_vals.append(mean1 - mean0)
        out_years.append(int(year))
...
var_raw_weighted = {}
var_raw_shifted = {}
if use_raw_seasonal_yearly:
    if not (proc_freq == 'Seasonal' and relation == 'Yearly' and months_dict is not None):
        raise ValueError('use_raw_seasonal_yearly=True only supports the seasonal yearly panel workflow.')

    raw_vars = set(vars_tendencies) | ({var_temp} if var_temp is not None else set())
    raw_years_load = set(years)
    raw_years_load.add(int(years[-1] + 1))
    if needs_extended_years(months_dict=months_dict):
        raw_years_load |= {int(y - 1) for y in years}
    raw_years_load = sorted(raw_years_load)

    print('='*60)
    print('PHASE 1B: Loading raw data for exact seasonal yearly panels')
    print('='*60)

    raw_weights = None
    for var in raw_vars:
        print(f'  Loading raw: {var}')
        data_raw = load_var(
            var=var,
            data_source='RACMO2.4A',
            data_sources=data_sources,
        ...
        if raw_weights is None:
            raw_weights = area_weights(
                data_raw.isel(time=0),
                rotpole_native=proj_cfg.get('RACMO2.4A', ccrs.PlateCarree())
            )

        var_raw_data[var] = data_raw

    print(f'Loaded {len(var_raw_data)} raw datasets\n')

    print('='*60)
    print('PHASE 2B: Domain averaging raw data')
    print('='*60)

    for var in raw_vars:
        print(f'  Area-weighting raw: {var}')
        var_raw_weighted[var] = area_weighted_mean(var_raw_data[var], weights=raw_weights).compute().astype('float64')
        if var in vars_tendencies:
            var_raw_shifted[var] = var_raw_weighted[var].shift(time=-1)
        del var_raw_data[var]

    var_raw_data = None
    print(f'Area-weighted {len(var_raw_weighted)} raw datasets\n')
...
if use_raw_seasonal_yearly:
    var_tend_raw_seasonal_yearly = {
        month_key: {
            var: raw_seasonal_yearly_from_shifted_tendency(var_raw_shifted[var], months_dict[month_key], years)
            for var in vars_tendencies
        }
        for month_key in PANEL_KEYS
    }
    var_temp_close_raw_seasonal_yearly = {
        month_key: raw_temperature_seasonal_yearly_change(var_raw_weighted[var_temp], months_dict[month_key], years)
        for month_key in PANEL_KEYS
    }

    group_panel_data = {
        group_name: {
            month_key: build_group_panel_data(group_name, var_tend_raw_seasonal_yearly[month_key])
            for month_key in PANEL_KEYS
        }
        for group_name in var_tend_main
    }
    closure_panel_data = {
        month_key: build_closure_panel_data(
            var_tend_raw_seasonal_yearly[month_key],
            var_temp_close_raw_seasonal_yearly[month_key],
        )
        for month_key in PANEL_KEYS
    }
...
# Option to just shift months instead.... So that december is start of the year
