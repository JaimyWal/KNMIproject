...
trend_calc = True
trend_crange = {'Tg': [-0.8, 0.8], 'SWin': [-4, 4]}
fit_against_gmst = False

# Correlation plotting arguments
corr_calc = False
corr_freq = 'Monthly' #
corr_crange = {'Tg': [0.95, 1], 'SWin': [0.95, 1]}
corr_cmap_type = None 

# RMSE plotting arguments
rmse_calc = False
rmse_freq = 'Monthly' #
rmse_crange = {'Tg': [0, 2], 'SWin': [0, 30]}
...
# Helper function to get color range for a variable
def get_crange(crange_cfg, var, default=None):
    if isinstance(crange_cfg, dict):
        return crange_cfg.get(var, default)
    return crange_cfg

# Build per-variable configuration
var_configs = {}
for var in var_list:
    ...
    # Make a copy of var_colors_cfg for this variable
    if not var_colors_cfg.get(var):
        var_colors = var_colors_cfg['Default'].copy()
    else:
        var_colors = var_colors_cfg[var].copy()
        if not var_colors.get('cmap_trend'):
            var_colors['cmap_trend'] = var_colors_cfg['Default']['cmap_trend']
            var_colors['extreme_trend'] = var_colors_cfg['Default']['extreme_trend']

    var_colors['cmap_mean'] = convert_cmap(
        var_colors['cmap_mean'], cmap_type, n_disc_colors
    )
    var_colors['cmap_trend'] = convert_cmap(
        var_colors['cmap_trend'], cmap_type, n_disc_colors
    )

    var_configs[var] = {
        'fit_unit': fit_unit,
        'fit_scaling': fit_scaling,
        'fit_x_label': fit_x_label,
        'label_unit': label_unit,
        'avg_label': avg_label,
        'avg_cmap_key': avg_cmap_key,
        'avg_ext_key': avg_ext_key,
...
cbar_pad = 0.008
cbar_thickness = 0.022
cbar_true_orientation = 'vertical'
if cbar_orientation == 'horizontal' and n_vars == 1:
    latlon_size = 16
    cbar_n_ticks = 11
    cbar_ticks_size = 26
    cbar_labelsize = 34
    cbar_true_orientation = 'horizontal'
    cbar_pad = 0.09
    cbar_thickness = 0.06
...
def make_cache_key(data_source, months, var):
    return (data_source, None if months is None else tuple(months), var)
...
for var in var_list:
    ...
    for ii in range(n_runs):
        ...
        if data_base_list[ii] is not None:
            ...
            cache_key_base = make_cache_key(data_base_list[ii], months_list[ii], var)
            ...
            if data_compare_list[ii] is None:
                ...
                cache_key_comp = make_cache_key(data_compare_list[ii], months_list[ii], var)
                ...
                if std_mask_ref_list[ii] is not None:
                    ...
                    if std_mask_ref_list[ii] == 'Pool':
                        ...
                        if std_mask_ref_list[ii] == data_base_list[ii]:
                            ...
                            cache_key_ref = make_cache_key(std_mask_ref_list[ii], months_list[ii], var)
...
all_area_contours = {}  # Dict of {var: [area_contours per run]}
...
for var in var_list:
    ...
    all_area_contours[var] = area_contours
...
if plot_climatology:
    ...
    fig_width = panel_width * n_runs
    fig_height = panel_height * n_vars
    ...
    # Add vertical spacing between rows to prevent tick overlap
    fig.get_layout_engine().set(h_pad=0.15)

    # Ensure axes is 2D array
    if n_vars == 1 and n_runs == 1:
        axes = np.array([[axes]])
    elif n_vars == 1:
    ...
    # Store meshes and data per variable for colorbars
    var_meshes = {}
    var_data_fields = {}
    var_row_axes = {}
    ...
    for var_idx, var in enumerate(var_list):
        ...
        results = all_results[var]
        area_contours = all_area_contours[var]
        cfg = var_configs[var]
        ...
        # Determine colorbar position based on alternating setting
        if alternating_colorbars:
            cbar_on_right = (var_idx % 2 == 0)
        else:
            cbar_on_right = True
        ...
        for ii, res in enumerate(results):
            ...
            if data_base_list[ii] is not None:
                ...
                title = res['title'] if var_idx == 0 else None
                
                # Determine y-label placement
                if alternating_colorbars:
                    if cbar_on_right:
                        # Colorbar on right, y-labels on left (first column)
                        show_y_labels = (ii == 0)
                        y_labels_side = 'left'
                    else:
                        # Colorbar on left, y-labels on right (last column)
                        show_y_labels = (ii == n_runs - 1)
                        y_labels_side = 'right'
                else:
                    show_y_labels = (ii == 0)
                    y_labels_side = 'left'
                ...
                show_x_labels = (var_idx == n_vars - 1)
                ...
                mesh, _ = plot_map(
                    ...
                    crange=get_crange(avg_crange, var, [0, 20]), 
                    label=cfg['avg_label'], 
                    cmap=cfg['var_colors'][cfg['avg_cmap_key']], 
                    extreme_colors=cfg['var_colors'][cfg['avg_ext_key']],
                    ...
                    y_labels_side=y_labels_side,
                    x_ticks_num=False,
                    x_ticks=10,
                    show_x_labels=show_x_labels,
                    tick_size=latlon_size,
        ...
        var_meshes[var] = meshes[0] if meshes else None
        var_data_fields[var] = data_avg_field
        var_row_axes[var] = row_axes

    # Add colorbars using shared_colorbar after all plots are done
    for var_idx, var in enumerate(var_list):
        cfg = var_configs[var]
        ...
        cbar_position = 'right' if cbar_on_right else 'left'
        
        if var_meshes[var] is not None:
            cbar = shared_colorbar(
                fig=fig,
                axes=var_row_axes[var],
                mesh=var_meshes[var],
                datasets=var_data_fields[var],
                crange=get_crange(avg_crange, var, [0, 20]),
                label=cfg['avg_label'],
                orientation=cbar_true_orientation,
                position=cbar_position,
                c_ticks=cbar_n_ticks,
                c_ticks_num=True,
                tick_labelsize=cbar_ticks_size,
                labelsize=cbar_labelsize,
                pad=cbar_pad,
                thickness=cbar_thickness
            )

    # out = Path.home() / 'KNMIproject' / 'RegionalTrends' / 'Main' / 'figuresproposal' / 'climatologyobs.pdf'
    # plt.savefig(str(out), 
    #             format='pdf', 
    #             bbox_inches='tight')
...
if trend_calc:
    ...
    for var_idx, var in enumerate(var_list):
        ...
        for ii, res in enumerate(results):
            ...
            if data_base_list[ii] is not None:
                ...
                trend_label = var_symbol_cfg[var] + ' trend (' + cfg['label_unit'] + ')'
                # if ii == 1:
                #     trend_label = var_symbol_cfg[var] + ' trend\n(' + var_units_cfg[var] + ' / ' + fit_unit + ')'
                ...
                mesh, _ = plot_map(
                    ...
                    crange=get_crange(trend_crange, var, [-1, 1]), 
                    label=trend_label, 
                    cmap=cfg['var_colors']['cmap_trend'], 
                    extreme_colors=cfg['var_colors']['extreme_trend'],
                    c_ticks=cbar_n_ticks,
                    show_x_ticks=True,
                    show_y_ticks=True,
                    y_ticks_num=False,
        ...
        var_meshes[var] = meshes[0] if meshes else None
        var_data_fields[var] = data_trend_field
        var_row_axes[var] = row_axes
    ...
    out = Path.home() / 'KNMIproject' / 'RegionalTrends' / 'Main' / 'figuresproposal' / 'trendobs.pdf'
    plt.savefig(str(out), 
                format='pdf', 
                bbox_inches='tight')
...
if corr_calc:
    ...
    for var_idx, var in enumerate(var_list):
        ...
        for ii, res in enumerate(results):
            ...
            if data_base_list[ii] is not None and data_compare_list[ii] is not None:
                ...
                corr_label = var_symbol_cfg[var] + ' correlation'
                ...
                mesh, _ = plot_map(
                    ...
                    crange=get_crange(corr_crange, var, [0.95, 1]), 
                    label=corr_label, 
                    cmap=corr_cmap, 
        ...
        var_meshes[var] = meshes[0] if meshes else None
        var_data_fields[var] = data_corr_field
        var_row_axes[var] = row_axes
    ...
    # out = Path.home() / 'KNMIproject' / 'RegionalTrends' / 'Main' / 'figuresproposal' / 'corrobs.pdf'
    # plt.savefig(str(out), 
    #             format='pdf', 
    #             bbox_inches='tight')
...
if rmse_calc:
    ...
    for var_idx, var in enumerate(var_list):
        ...
        for ii, res in enumerate(results):
            ...
            if data_base_list[ii] is not None and data_compare_list[ii] is not None:
                ...
                rmse_label = var_symbol_cfg[var] + ' RMSE (' + var_units_cfg[var] + ')'
                ...
                mesh, _ = plot_map(
                    ...
                    crange=get_crange(rmse_crange, var, [0, 2]), 
                    label=rmse_label, 
                    cmap=rmse_cmap, 
        ...
        var_meshes[var] = meshes[0] if meshes else None
        var_data_fields[var] = data_rmse_field
        var_row_axes[var] = row_axes
    ...
    # out = Path.home() / 'KNMIproject' / 'RegionalTrends' / 'Main' / 'figuresproposal' / 'rmseobs.pdf'
    # plt.savefig(str(out), 
    #             format='pdf', 
    #             bbox_inches='tight')
...
# OPTIE VOOR HORIZONTALE COLORBAR ZOALS IN HET VORIGE SCRIPT!